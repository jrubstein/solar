...
export class LoggerFactory {
  constructor(@inject(TYPES.ApplicationConfiguration) private configuration: ApplicationConfigurationType) {}
  ...
  private getConfiguration(name: string) {
    ...
    return {
      ...
      format: combine(label({ label: name }), timestamp(), fromatLogLine),
    }
  }
}
